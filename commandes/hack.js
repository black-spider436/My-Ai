...
bravin({ nomCom: "hack", categorie: "General", reaction:"👨‍🏫", active: isHackCommandEnabled }, async (dest, zk, commandeOptions) => {
  const { ms, arg, repondre } = commandeOptions;
  const message = arg.join(' ');
  // hack
    const { repondre, arg, ms } = commandeOptions;

    await zk.sendMessage(dest, "```My Ai Injecting malware```");
    await sleep(30000);
